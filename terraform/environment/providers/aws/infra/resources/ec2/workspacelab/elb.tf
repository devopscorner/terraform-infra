# ==========================================================================
#  Resources: WorkspaceLab / elb.tf (Classic LoadBalancer)
# --------------------------------------------------------------------------
#  Description
# --------------------------------------------------------------------------
#    - ELB Tags
...
locals {
  elb_tags = {
    ...
    ResourceGroup = "${var.environment[local.env]}-ELB-WORKSPACELABLAB"
  }
}

resource "aws_elb" "workspacelab" {
  name = "${var.ec2_name}-elb-${var.env[local.env]}"
  subnets = [
    ## Public Subnet
  ...
  instances                   = ["${aws_instance.workspacelab.id}"]
