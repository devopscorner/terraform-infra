...
output "instance" {
  value = {
    ...
    ssh_access_dns = "${var.ami}" == "ami-0dc5785603ad4ff54" ? "ssh ec2-user@${aws_eip.airflow.public_dns}" : "ssh ubuntu@${aws_eip.airflow.public_dns}"
    ssh_access_ip  = "${var.ami}" == "ami-0dc5785603ad4ff54" ? "ssh ec2-user@${aws_eip.airflow.public_ip}" : "ssh ubuntu@${aws_eip.airflow.public_ip}"
